...
var draw2d_circuit_display_7Segment = draw2d.SetFigure.extend({            
   ...
   createSet: function()
   {
       this.canvas.paper.setStart();
        ...
        // circle
        shape = this.canvas.paper.path('M5.656099356607842,3.9933553850511885Q5.886561433060251,0 9.886561433060251, 3.077116553600654e-14L61,4.239761853504576e-13Q65,4.547473508864641e-13 64.75986973301833, 3.9927856760516107L59.10574459758781,98.00721432394884Q58.86561433060615,102 54.86561433060615, 102L4,102Q0,102 0.23046207645240901, 98.00664461494881L5.656099356607842,3.9933553850511885');
...
var draw2d_circuit_gate_AND = draw2d.SetFigure.extend({            
   ...
   init:function(attr, setter, getter)
   {
     ...
     var port;
     // Port
     port = this.createPort("input", new draw2d.layout.locator.XYRelPortLocator(0, 22.5));
     port.setConnectionDirection(3);
     port.setBackgroundColor("#1C9BAB");
     port.setName("Port");
     // Port
     port = this.createPort("input", new draw2d.layout.locator.XYRelPortLocator(0, 77.5));
     port.setConnectionDirection(3);
     port.setBackgroundColor("#1C9BAB");
     port.setName("Port");
     // Port
...
var draw2d_circuit_gate_NAND = draw2d.SetFigure.extend({            
   ...
   init:function(attr, setter, getter)
   {
     this._super( $.extend({stroke:0, bgColor:null, width:58,height:61},attr), setter, getter);
     var port;
     // Port
     ...
     port.setName("Port");
     // Port
     port = this.createPort("output", new draw2d.layout.locator.XYRelPortLocator(106.89655172413792, 50));
     port.setConnectionDirection(1);
     port.setBackgroundColor("#1C9BAB");
     port.setName("Port");
     this.persistPorts=false;
   ...
   createSet: function()
   {
       this.canvas.paper.setStart();
        ...
        // BoundingBox
        shape = this.canvas.paper.path("M0,0 L58,0 L58,61 L0,61");
        shape.attr({"stroke":"none","stroke-width":0,"fill":"none"});
        shape.data("name","BoundingBox");
        ...
        // Circle
        shape = this.canvas.paper.ellipse();
        shape.attr({"rx":7,"ry":7,"cx":51,"cy":30.5,"stroke":"#1B1B1B","stroke-width":1,"fill":"#FFFFFF","opacity":1});
        shape.data("name","Circle");
